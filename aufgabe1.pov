...
declare x_size = 60;
declare z_size = 60;     // Gr��e der Halle
declare height = 8;
...
declare Messhalle_mit_Eingang = 
    union{
        ...
        object{Eingang}
        //    Messehallenflur, geflie�t
        box {
            pos_halle1+<0.1,0,0.1>,
            pos_halle2-<0.1,7.9,0.1>
            pigment{ checker color rgb<1,1,1> color rgb<0.9,0.9,0.9> } // geflie�ter Boden
        }  
    }
...
//  ************  Gegenst�nde fuer Messestand modellieren *************************
...
declare poster = union{
    box{<0,1.3,-0.5><0.01,2.3,0.5> texture{T_Wood12}}          // Plakatfl�che
    // Standfoto, wird �ber x-y Fl�che aufgetragen, erst dann rotieren!
    box{<0,0,0><1,1,0>  texture { pigment { image_map { png "texture1"}}}  rotate<0,-90,0> translate<0.02,1.3,-0.5>} 
    union{  
        cylinder{ <0,0,0>,<0,1.5,0>,0.01 pigment{ color Black}}  // St�nder
        cylinder{ <0,0,0>,<0,0.01,0>,0.2 texture{T_Wood12}}      // Standfu�
    }
}

// Spotlight, von der Decke h�ngend
declare lampe = union{  // Spotlight, Hallendecke 8m, bis auf 2.50 runter
    cylinder{<0,3,0>,<0,8,0>,0.005 pigment{color Black}}
    difference{ // innerer, au�erer Kegel f�r Lampe
...
// da so keine absoluten Positionsdaten berechnet werden m�ssen, sondern nur Gr��enangaben!
...
declare card_reader = difference{ 
    ...
    union{
         // SD Karte Slot : 25x3x30 --> Etwas gr��er, aber nicht so tief wie SD-Karte gro� ist.
         ...
         // langer Kartenslot, frei erfunden 2,.. x SD Karten Gr��e
...
declare foot1 = box_start+<0.08,-0.01,0.05>;    // relative Fu�position
declare foot2 = foot1+<-2*0.08+0.4,0,0>;        // relative Fu�position 2
...
union{    
    ...
    union{
        ...
        // zylindrische F��e 5 h�he x 2 durchmesser, relativ
...
// Vorlage um alle Einbausch�chte auszuschneiden
...
// Zus�tzlich kleinen Bildschrim hinzuf�gen
box{box_start+<0.12,0.068,-0.0005> box_start+<0.28,0.105,0.02> pigment{color rgb<0,0.3,0.9>}}
}

// Mediabox besteht dann aus Box ohne Schnittstellen + Einbausch�chte + Ger�te
...
// Mediabox aufstellen, Tischh�he: 1.25 + 0.05 Fu�h�he von der Mediabox
